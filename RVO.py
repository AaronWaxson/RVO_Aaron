...
def RVO_update(X, V_des, V_current, ws_model):
    ...
    for i in range(len(X)):
        ...
        pA = [X[i][0], X[i][1]]
        # RVO consisted by Obj_A and Obj_B
        RVO_BA_all = []
        for j in range(len(X)):
            if i!=j:
                ...
                bound_left = [cos(theta_ort_left), sin(theta_ort_left)] # uint vector of bound
    ...
    for theta in numpy.arange(0, 2*PI, 0.1): # sample vel vector
        for rad in numpy.arange(0.02, norm_v+0.02, norm_v/5.0): # sample vel scalar
            ...
            for RVO_BA in RVO_BA_all:
                ...
                left = RVO_BA[1]
                right = RVO_BA[2]
                dif = [new_v[0]+pA[0]-p_0[0], new_v[1]+pA[1]-p_0[1]] # vA-vB
...
def compute_V_des(X, goal, V_max):
    ...
    for i in range(len(X)):
        ...
        # norm_dif_x = [dif_x[k]*V_max[k]/norm for k in range(2)] # origin
        # compute max velocity of direction of agent to goal
        norm_dif_x = [dif_x[k]*V_max[i]/norm for k in range(2)] # Aaron version
